...
public class SVDandFT {
	...
	public static void invertSVD(SimpleMatrix A)
	{
		...
		for(int i = 0; i < Sinv.getRows(); i++){
			double val = 1.0/ svd.getS().getElement(i, i);  //compute inverse of the singular value
			Sinv.setElementValue(i,i,val); //insert in matrix
		}
		...
		// set all single values that are smaller than epsilon to zero
		double eps = 10e-3;
		...
		for(int i = 0; i < sInd; i++)
		{
			...
			if(val > eps){
				Slowrank.setElementValue(i, i, val); //check the rank deficicency with the threshold
			}
		}
	...
	public static void main(String[] args) {
		...
		String filename = "/proj/i5dmip/ro75xevi/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/yu_fill.jpg";
		...
		String filenameShepp = "/proj/i5dmip/ro75xevi/Reconstruction/CONRAD/Shepp.tif";
